...
4. No contract path allows token-denominated default wages.
...
Acceptance:
1. Every accepted job emits enough metadata to reconstruct reward and receipt state.
2. Broker can publish epoch reward allocations deterministically.
...
2. Worker always sees stable payout separately from token rewards.
3. Premium claims clearly show required stake and slash risk.
...
3. `WorkReceipt1155` must ship before any derivative vault.
4. `AIU` index methodology must be published before any forward-style product.

### Suggested Milestones
1. Milestone A
   - stablecoin payout and fee routing finalized
2. Milestone B
   - worker stake and buyer access tiers live
3. Milestone C
   - epoch reward distribution live
4. Milestone D
...
4. No open public forward market on day one.
5. No derivative pool without receipt inventory and cash-flow logic.
...
4. Whether reward top-ups come from fee routing, buybacks, or both.
5. When to allow the first closed-beta derivative vault.
